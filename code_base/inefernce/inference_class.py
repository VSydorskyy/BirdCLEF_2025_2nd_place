...
class BirdsInference:
    ...
    def _model_forward(
        ...
        if is_onnx_model:
            model_out = torch.from_numpy(nn_model.run(
                None,
                {"input": wave.numpy()},
            )[0])
            logits = model_out
    ...
    def _model_forward_test(
        self, nn_models, wave, to_cpu=True, is_onnx_model=False, is_openvino_model=False
    ):
        ...
        for wave, target, dfidx, start, end in self._tqdm_v(data_loader):
            ...
            if ansamble:
                ...
                (
                    ...
                    nn_models=nn_models,
                    wave=wave,
                    to_cpu=not self.use_sed_mode and not (is_onnx_model or is_openvino_model),
                    is_onnx_model=is_onnx_model,
                    is_openvino_model=is_openvino_model,
                )
